...
Placeholders and substutitions
\begin{enumerate}

\item Untyped bound variables: Types of unused untyped bound variables do not
  get substititutions. Every type variable of a used untyped bound variable
  gets a substitution or is used in the substitution of onother placeholder.

\item Inferable implicit type variables: ?

\item Proof arguments: Get substitutions from the prover

\item Placeholder for (sub)terms: These placeholders are guaranteed to get
  substitutions.

\item Type variable for the expected type of a toplevel expression: Since
  every expression has a type, the type variable gets a substitution. But the
  type of the expression might contain other type variables or might be a type
  variable itself.
