...
\begin{alba}
  ...
  -- left folding
  fold_left A (a: List A) B (f: A -> B -> B) (s:B): B :=
    inspect a case
      [] :=
        s
      h ^ t :=
...
The type $E$ is the required result type of the expression applied to its
arguments. It might be a placeholder without substitution if we do not know
...
surround the explicit arguments.

We unify $R$ with the expected type, introduce the placeholders
$x^A, y^B, \ldots$, substitute the placeholder for the expression with
$v x y \ldots$ where $v$ is the variable with the placeholder for the
expression and return the new context with an array pointing to the
placeholders representing arguments.
