...
with actual type $A$. We can fill the hole $x$ by $e$ only if $R$ is a supertype
of $A$ i.e. if $A \le R$ is valid. Since we do not yet know the validity, we can
only state the requirement $A \lemust R$ which reads \emph{The holes in $A$ and
$R$ must be filled such that the substituted $A$ is a subtype of the substituted
$R$}.
...
\begin{description}
\item[sort-sort]
Sorts can only be unified if the figure in the required relation.
$$
\rulev {
