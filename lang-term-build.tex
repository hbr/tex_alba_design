...
\item[Ambiguity]
  ...
  global function, it has to do the buildwith the different functions in
  parallel until a function is rule out because of the typing rules.
...
At the start of a term building we introduce two holes into the context. One
($E$) for the type of the term to be constructed and one ($e$) for the term to
be constructed.  We start with the context
$$
\Gamma, E^{\Any_2}, e^E
$$

We choose the type of $E$ to be $\Any_2$ in order to be able to fill the hole
$e$ with $\Any_0$, which requires the filling of $E$ with $\Any_1$ which
requires the type of $E$ to be $\Any_2$.


If we expect a type we use a placeholder $T^{\Any_1}$. This makes the filling of
$T$ with any type e.g. with $\Natural$, $a \le b$, $\Any_0$, or $\Prop$
possible.
...
x}_B$ and unify $B$ with $E$. If there are more implicit arguments, the same
...
$$
\Gamma, \ldots, e^E, \ldots, A^{\Any_1}, a^A, F^{A\to\Any_1}, f^{\Pi x^A. F x}
$$
and the constraint
$$
f a \ldots : E
...
$$
\Gamma, \ldots, e^E, \ldots, A^{\Any_1}, F^{A\to\Any_1}, x^A, T^{\Any_1}, t^T
$$
%
...
If the result type is given explicitly, we analyze it and fill the hole $T$.

Then either we assign $T := F x$ or $F := \lambda x^A. T$.

If $E$ is not a placeholder it must have the form
$$
\Pi x^{A_r}. B_r
$$

In that case we unify $A \sim A_r$ and either set $F := \lambda x^A. B_r$ or
unify $T \le B_r$ in case that $F$ has already been filled.


Then we analyze $t$ and fill the hole.
