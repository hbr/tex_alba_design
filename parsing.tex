...
\subsubsection{Combinator Processing One Token}
...
\code{token} function extracts the lookahead token and the current state from
...
\code{token} and \code{fail}.


\subsubsection{Alternatives}


The function \code{token} adds and error message, if the lookahead token is
not the token it had expected. In that error case, the token is not consumed
and stays in the lookahead buffer.

If the rejected token is the first token of a parser \code{p} which is part of
an alternative \code{p <|> q}, then the alternative parser \code{q} is
...
If all alternatives at a certain position fail without consuming the lookahead
...
In all these parser it can happen that the parser \code{p} fails without
...
I.e. consider a combinator which parses a certain construct followed by zero
...
keyword \code{if}, etc. If the parser encounters a colon (\code{:}) the error
message could point to the colon and read like ``expecting ')' or operator or
\code{if}'' i.e. displaying to the user all ways how a legal expression could
...
\item \code{token}: This combinator either consumes a token or rejects it as
  unexpected. In the latter case it adds an error message indicating what it
  had expected.

\item \code{p <|> e}: This combinator parses \code{p} and in case of a error
  without consuming token it replaces the list of error messages generated by
  \code{p} by a single error message expressing the expectation at a higher
  level of abstraction.

\item \code{backtrackable p e}: This combinator parses \code{p} and in case
  that \code{p} fails even if it had consumed token, it backtracks to the
  previous state (pretending that no token had been consumed) and adding the
  expectation \code{e} to the errors.
\end{enumerate}
...
The only combinator which generates a semantic error is \code{fail}. This
...
it does not satisfy a certain criteria it can be reject by calling \code{fail
  e}. This usually means that the error occurred before the current position
