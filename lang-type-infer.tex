...
\item[Implicit Proof Arguments]
  ...
  \end{alba}
  %
  then the user can call the function with
  \begin{alba}
    predecessor n
  \end{alba}
  %
  ...
  (e.g. $p: 0 < n$) and generates the term $\text{predecesssor n p}$.
  %
...
\item[Specified Type Splitting]
  ...
  type. The simple type and the proposition have to be provided as separate
  arguments. But the objects of specified type can be returned from functions
  (e.g. a number with a proof of the fact that it is a prime number).
  ...
  \begin{alba}
     ...
     proof (A:Any) (f:A -> Proposition) (v:Specified f): A :=
       inspect v case
         specified _ p := p
  \end{alba}
  %
