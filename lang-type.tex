...
\item[Pattern match] $\case(e,\lambda x^T.R, \fbold)$ A pattern match term has
  ...
  The subterm $\lambda x^T.R$ is the elimination function. It determines the
  ...
  function becomes important as soon as each case returns an object of a
  different type. As described later in the section of propositional
  types~\ref{sec:propositional-types} the elimination function needs not only
