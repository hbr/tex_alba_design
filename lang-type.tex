...
\begin{definition}
  ...
  $$
  \lambda \ybold^\Bbold x^T.R : \Pi \ybold^\Bbold. s_R
  $$
  is a
  %
...
type we need a bound variable as a representant of the type and a context
describing postulated properties of that type
$$
\Abstract_{T:K, \Gamma}
$$
where $K$ is a kind (i.e. $T$ can represent a type constructor needing
arguments), $\Gamma = [d_0:D_0, d_1: D_1, \ldots]$ and the bound variable $T$
occurs in each $D_i$.



\subsubsection{Example: Group}
The type of the bound variable $T$ of a group is $\Any$ i.e. it represents a
type which is the type of a group element. The context $\Gamma$ is defined as
$$
\begin{array}{llll}
  \Gamma =
  &[
...
The type of the bound variable $T$ of a sortable is $\Any$ i.e. it represents
a type which is the type of sortable elements. The context $\Gamma$ is defined
as
$$
\begin{array}{llll}
  \Gamma =
