...
\begin{alba}
    ...
    run: T -> T := case
        \ (co f) :=
            -- any term return by 'f' is considered 'smaller'
            run (f (co f))
\end{alba}
...
This behavior of the $\Any$ is called predicativity. In general, predicativity
...
\begin{alba}
    ...
    _: Adt q1 q2 ... :=
        adt ...  -- provide the attributes
\end{alba}




\subsubsection{Example: Decidable}

\begin{alba}
    abstract class
        Decidable (A: Any): Any
    :=
        isEqual: all (x y: A): Decision (Equal x y)
    ...
    :=
        case
            -- cannot match, no case

    (=?)
        {A: Any} {d: Decidable A}
        (xs ys: List A)
        : Decision (Equal xs ys)
    :=
        inspect xs, ys case
            [], [] :=
                left identical
            [], _ :: _ :=
                right nilNeqCons
            _ :: _, [] :=
                right (equalSymmetric >> nilNeqCons)
            x :: xs, y :: ys :=
                inspect
                    isEqual d x y, xs =? ys
                case
                    left eqXY, left eqXsYs :=
                        ...
                    ...
                    ...

    _: Decidable (List Int) :=
        decidable (=?)
\end{alba}
