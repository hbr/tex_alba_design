...
\end{alba}
%
Problem: \code{Fix} occurs in \code{F (Fix F)} in a positive position, but it is
nested in \code{F}. At the time of compilation the compiler does not know that
the type constructor \code{F} is an inductively defined type constructor. This
is known only a instantiation time.

Possible solution: Mark the parameter \code{F} is a parameter which can be
