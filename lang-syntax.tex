...
When the upper node in the syntax tree has higher precedence, then parentheses
...
The function \code{ack} satisfies the specification of the Ackermann
function. Proof
\begin{itemize}

\item For $m = 0$ or $n = 0$ it is easy to see by expanding the definitions.

\item For $m = 1+k$ and $n = 1+o$ we use the equalities
  \begin{alba}
...
It is possible to define the Ackermann functions based on \code{iterations} only.
